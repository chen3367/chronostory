    ...
    searchInput.addEventListener('input', function(e) {
        ...
        if (searchTerm === '') {
            ...
            debounceTimer = setTimeout(() => {
                ...
                } else if (window.location.pathname.includes('translate_item.html')) {
                    if (typeof searchTranslateItems === 'function') {
                        searchTranslateItems(searchTerm);
                    }
                } else if (window.location.pathname.includes('translate_mob.html')) {
                    if (typeof searchTranslateMobs === 'function') {
    ...
    if (translateInput && translateButton) {
        ...
        translateInput.addEventListener('input', function(e) {
            ...
            translateInput.debounceTimer = setTimeout(() => {
                // 檢查是否在怪物翻譯頁面
                if (window.location.pathname.includes('translate_mob.html')) {
                    searchTranslateMobs(searchTerm);
                } else {
                    searchTranslateItems(searchTerm);
                }
...
async function performTranslation(text, mode) {
    ...
    // 檢查是否在怪物翻譯頁面
    const isMobTranslation = window.location.pathname.includes('translate_mob.html');

    // 如果沒有選中的ID，嘗試從名稱查找（備用邏輯）
    if (!itemId) {
        if (isMobTranslation) {
            itemId = getTranslateMobIdByName(text);
        } else {
        ...
        try {
            ...
            if (window.location.pathname.includes('translate_mob.html')) {
                ...
                if (mode === 'chToEn') {
                    ...
                    apiUrl = `https://maplestory.io/api/GMS/62/mob/${itemId}`;
                    console.log('獲取怪物英文名稱:', apiUrl);
                } else if (mode === 'enToCh') {
                    // 英翻中模式：使用TWMS API獲取中文名稱
                    apiUrl = `https://maplestory.io/api/TWMS/256/mob/${itemId}`;
            ...
            if (response.ok) {
                const data = await response.json();
                const resultName = isMob ? data.name : (data.description ? data.description.name : data.name);
                translateResultDisplay.innerHTML = `
                    <div class="translate-result">
                        <div class="translate-original">原文: ${text}</div>
